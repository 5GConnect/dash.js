...
var app = angular.module('DashPlayer', ['angular-flot']); /* jshint ignore:line */

$(document).ready(function() {
    $('[data-toggle="tooltip"]').tooltip();
});

app.controller('DashController', function($scope, $timeout, $q, DownloadService) {
    ...
    function updateMetrics(type) {
        ...
        if (dashMetrics && $scope.streamInfo) {
            ...
            if (type == 'video') {
                let bitrateInfo = $scope.player.getBitrateInfoListFor('video')[index]
                $scope.videoQuality = `${bitrateInfo.width}x${bitrateInfo.height} px`
            }
            var liveLatency = 0;
            if ($scope.isDynamic) {
